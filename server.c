...
static void read_from_socket(int client_sockfd,
                             struct sockaddr_storage *client_addr,
                             char *buffer);
...
static void parse_arguments(const int argc, char *argv[], char **ip_address,
                            char **port) {
  int opt;
  opterr = 0;
...
static in_port_t parse_in_port_t(const char *binary_name,
                                 const char *port_str) {
  char *endptr;
...
static int socket_accept_connection(int server_fd,
  ...
  char client_service[NI_MAXSERV]; // Array to store the port information of the
                                   // client
...
static void read_from_socket(int client_sockfd,
  ...
  //  const int min_request_length = 16;
  ...
  while (!key_found) {
    ...
    //    if (strlen(buffer) >= min_request_length) {
    // Check if the end of the header (\r\n\r\n) is in the buffer
    ...
    //      fprintf(stderr, "Request is too large for buffer\n");
    //      return; // Buffer overflow, may need to resizing buffer or handle as
    //      error
    //    }
  }
...
#pragma clang diagnostic pop
  ...
  sa.sa_flags =
      0; // Set sa_flags to 0, indicating no special flags for signal handling.
